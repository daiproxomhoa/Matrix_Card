 ...
 */
export class Invite extends Sprite {
    ...
    getList = (guest: any[]) => {
        ...
        for (let i = 0; i < guest.length; i++) {
            ...
            if (guest.length < 6) {
                if (i % 2 == 0)
                    btn = new Button(-145, 110 * Math.floor(i / 2) - 75, guest[i].name, App.AssetDir + "Picture/IU/inviteArea.png");
                else
                    btn = new Button(145, 110 * Math.floor(i / 2) - 75, guest[i].name, App.AssetDir + "Picture/IU/inviteArea.png");
                ...
                if (i % 3 == 0)
                    btn = new Button(-195, 70 * Math.floor(i / 3) - 80, guest[i].name, App.AssetDir + "Picture/IU/inviteArea.png");

                else if (i % 3 == 1)
                    btn = new Button(0, 70 * Math.floor(i / 3) - 80, guest[i].name, App.AssetDir + "Picture/IU/inviteArea.png");
                else
                    btn = new Button(195, 70 * Math.floor(i / 3) - 80, guest[i].name, App.AssetDir + "Picture/IU/inviteArea.png");
